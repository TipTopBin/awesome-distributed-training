...
get_imds_info() {
    ...
    # 方法1: IMDSv1
    result=$(curl -s "http://169.254.169.254/latest/meta-data/$endpoint" 2>&1)
    
    # 方法2: IMDSv2
    if [ -z "$result" ] || [[ "$result" == *"error"* ]]; then
        local token=$(curl -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600" -s 2>&1)
        if [ ! -z "$token" ] && [[ "$token" != *"error"* ]]; then
            result=$(curl -H "X-aws-ec2-metadata-token: $token" -s "http://169.254.169.254/latest/meta-data/$endpoint" 2>&1)
        fi
    fi
...
# 设置 AWS 区域信息（统一获取一次）
export AWS_REGION=$(get_imds_info "placement/region")
MY_AZ=$(get_imds_info "placement/availability-zone")
[ -z "$AWS_REGION" ] && [ ! -z "$MY_AZ" ] && AWS_REGION=${MY_AZ%?}
if [ -z "$AWS_REGION" ]; then
    echo "⚠️  无法获取 AWS 区域信息"
else
...
# Check current root disk size
ROOT_SIZE_GB=$(df -BG / | awk 'NR==2 {print $2}' | sed 's/G$//' | tr -d ' ')
echo "当前根盘大小: ${ROOT_SIZE_GB}GB"

# Check if root disk is smaller than 200GB
if [[ "$ROOT_SIZE_GB" =~ ^[0-9]+$ ]] && [ "$ROOT_SIZE_GB" -lt 200 ]; then
    echo "⚠️  根盘大小 (${ROOT_SIZE_GB}GB) 小于 200GB，尝试扩容..."
    
    # Download power-ebs.sh script
    if [ ! -f "$CUSTOM_DIR/bin/power-ebs.sh" ]; then
        mkdir -p "$CUSTOM_DIR/bin"
        if curl -s -o "$CUSTOM_DIR/bin/power-ebs.sh" https://raw.githubusercontent.com/TipTopBin/awesome-distributed-training/main/utils/power-ebs.sh; then
            chmod +x "$CUSTOM_DIR/bin/power-ebs.sh"
            echo "✅ power-ebs.sh 下载成功"
        else
            echo "❌ power-ebs.sh 下载失败"
        fi
    fi
    
    # Check EC2 permissions and resize
    if [ -f "$CUSTOM_DIR/bin/power-ebs.sh" ] && aws ec2 describe-volumes --max-items 1 >/dev/null 2>&1; then
        echo "执行磁盘扩容: ${EBS_SIZE}GB..."
        if "$CUSTOM_DIR/bin/power-ebs.sh" "$EBS_SIZE" "$EBS_IOPS" "$EBS_THROUGHPUT" >/dev/null 2>&1; then
            # Verify resize
            sleep 2
            NEW_SIZE=$(df -BG / | awk 'NR==2 {print $2}' | sed 's/G$//' | tr -d ' ')
            if [[ "$NEW_SIZE" =~ ^[0-9]+$ ]] && [ "$NEW_SIZE" -ge 100 ]; then
                echo "✅ 根盘扩容成功: ${NEW_SIZE}GB"
            else
                echo "⚠️  扩容可能失败，当前: ${NEW_SIZE}GB"
            fi
        else
            echo "❌ 磁盘扩容执行失败"
        fi
